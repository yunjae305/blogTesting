...
def brand_utility_rules(blog_input: BlogTaskInput) -> list[str]:
    ...
    # 쓸 수 있는 기능 이름. 저장 시점에 소재와 닿은 줄만 실린다(with_brand_materials).
    if blog_input.brand_use_cases:
        lines.append(
            "- 이 소재에서 쓸 수 있는 기능(브랜드 자료의 기준표에서 이 소재에 닿은 줄이다."
            " **이 이름 그대로** 쓰고, 목록에 없는 기능을 지어내지 않는다):"
            + "\n"
            + "\n".join(f"  {line}" for line in blog_input.brand_use_cases)
        )
    else:
        lines.append(
            "- 기능 이름은 **참고자료의 브랜드 자료에 적힌 것만** 쓴다. 자료에서 이름을"
            " 확인할 수 없으면 기능명을 만들어 붙이지 말고, 무엇을 했는지만 쓴다"
    ...
    elif blog_input.brand_fit_grade == BRAND_FIT_FORCED:
        # 화면이 이 조합을 막지만, 사용자가 굳이 넘겼다면 최소한 겉돌지는 않게 한다.
        lines.append(
            "- 이 소재와 도구는 자연스럽게 닿지 않는다고 판정됐다. 억지로 이으려 하지"
            " 말고, 소재에 대한 글을 온전히 쓴 뒤 마지막에 한 번만 짧게 언급한다."
            " 연결이 어색하다면 아예 언급하지 않는 편이 낫다.",
        )
