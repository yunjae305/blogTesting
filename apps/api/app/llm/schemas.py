...
#: 사이트를 읽어 브랜드 자료를 **제안**할 때의 응답 모양(2026-08-20).
#:
#: 저장 모양(`validate_brand_body`)과 칸 이름을 맞춰 둔다 — 화면이 받은 것을 그대로
#: 편집 칸에 부어야 하는데, 이름이 다르면 그 사이에 옮겨 담는 코드가 한 벌 더 생긴다.
#:
#: 못 찾은 것은 **빈 값**으로 온다. 지어내는 것보다 비는 편이 낫다: 비면 사람이 채우면
#: 되지만, 지어낸 기능 이름은 그대로 원고에 실린다.
BRAND_IMPORT_SCHEMA = {
    "type": "object",
    "additionalProperties": False,
    "required": ["description", "features", "useCases", "links"],
    "properties": {
        "description": {
            "type": "string",
            "description": "What the brand is. Korean prose, 2-6 sentences. Empty if the site does not say.",
        },
        "features": {
            "type": "string",
            "description": "Core features and services. Korean prose. Use the exact product names printed on the site.",
        },
        "useCases": {
            "type": "array",
            "description": (
                "Situation -> feature rows. Only rows whose feature name is literally printed "
                "on the site. Never invent a feature name."
            ),
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["situation", "feature", "keywords"],
                "properties": {
                    "situation": {
                        "type": "string",
                        "description": "Reader's situation, in Korean. e.g. '어떤 정보를 알아보고 싶을 때'",
                    },
                    "feature": {
                        "type": "string",
                        "description": "Exact feature name as printed on the site. e.g. '자료 조사'",
                    },
                    "keywords": {
                        "type": "array",
                        "description": "Korean words a reader would search for this situation.",
                        "items": {"type": "string"},
                    },
                },
            },
        ...
        "links": {
            "type": "array",
            "description": "Pages worth keeping as references.",
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["label", "url"],
...
GEMINI_BRAND_IMPORT_SCHEMA = to_gemini_schema(BRAND_IMPORT_SCHEMA)


#: 신기능 페이지 하나를 읽어 **글의 출발점**으로 바꿀 때의 응답 모양(2026-08-20).
FEATURE_BRIEF_SCHEMA = {
    "type": "object",
    "additionalProperties": False,
    "required": ["name", "summary", "highlights", "keywords"],
    "properties": {
        "name": {
            "type": "string",
            "description": (
                "The feature's name exactly as printed on the page. This becomes the "
                "article's topic, so it must be the real name, not a description."
            ),
        },
        "summary": {
            "type": "string",
            "description": "What is new about it, in Korean. 2-4 sentences.",
        },
        "highlights": {
            "type": "array",
            "description": "What a reader can actually do with it. Korean. 3-6 items.",
            "items": {"type": "string"},
        },
        "keywords": {
            "type": "array",
            "description": "Korean search terms a reader would use to find this.",
            "items": {"type": "string"},
        },
    },
}

GEMINI_FEATURE_BRIEF_SCHEMA = to_gemini_schema(FEATURE_BRIEF_SCHEMA)
