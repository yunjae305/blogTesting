...
describe("StepTopic 소재 분야와 브랜드", () => {
  ...
  /**
   * 결합 가능성 판정(2026-08-19). 얹을 수 있다는 것과 얹어야 한다는 것은 다르다 —
   * 브랜드를 쓸 이유가 없는 소재에 붙이면 남는 것은 광고 문장뿐이다.
   */
  async function withFit(grade: "A" | "B" | "C", reason = "판정 이유") {
    mocks.request.mockImplementation(async (path: string) => {
      if (path.endsWith("/fit")) return { grade, reason, features: ["자료 조사"], matches: [] };
      ...
      return { postId: "post_1", input: {} };
    });
  }

  /** 판정은 손이 멈춘 뒤에 한 번만 묻는다(0.5초). 그 시간을 건너뛴다. */
  ...
  it("소재와 브랜드가 닿는 정도를 저장하기 전에 보여 준다", async () => {
    vi.useFakeTimers();
    try {
      await withFit("A", "소재가 곧바로 닿습니다");
      await render();
      await typeTopic("다이어트 간식");
      await pickBrand();
      await settleFit();

      const fit = container.querySelector(".brand-fit");
      expect(fit?.getAttribute("data-grade")).toBe("A");
      expect(fit?.textContent).toContain("바로 연결");
      expect(fit?.textContent).toContain("소재가 곧바로 닿습니다");
      // 어느 기능을 쓸 글인지도 그 자리에서 보여 준다.
      expect(fit?.textContent).toContain("자료 조사");
    } finally {
      vi.useRealTimers();
    }
  });

  it("억지 조합(C)은 한 번 막고, 다시 누르면 그대로 저장한다", async () => {
    // 판정은 브랜드 기준표로 재는 것이라 표에 없는 상황은 C로 나온다. 끝까지 막으면
    // 사용자가 그 판단을 뒤집을 방법이 없다(2026-08-19).
    vi.useFakeTimers();
    try {
      await withFit("C", "이을 상황이 없습니다");
      await render();
      await typeTopic("강아지 산책 코스");
      await pickBrand();
      await settleFit();
      await act(async () => {
        container
      ...
      expect(savedBody()).toBeUndefined();
      expect(mocks.store.showToast).toHaveBeenCalledWith(
        expect.stringContaining("자연스럽게 이어지지 않습니다"),
        true,
      );

      await submit();
      expect(savedBody()?.topic).toBe("강아지 산책 코스");
    } finally {
      vi.useRealTimers();
    }
  });

  it("AIONA 앱스튜디오에서 들어오면 그 브랜드를 미리 골라 둔다", async () => {
    // 그 사람이 하려는 일은 정해져 있다 — 매번 목록에서 같은 브랜드를 다시 고를 이유가 없다.
    const before = window.location.hash;
    window.location.hash = "#/write?campaign=aiona";
    try {
      await render();

      expect(container.querySelector<HTMLSelectElement>("#brandId")?.value).toBe("brand_1");
      // 소재는 건드리지 않는다. 무엇에 대해 쓸지는 여전히 사용자가 정한다.
      expect(container.querySelector<HTMLInputElement>("#topic")?.value).toBe("");
    } finally {
  ...
  it("저장된 글을 다시 열면 캠페인이 그 선택을 덮지 않는다", async () => {
    const before = window.location.hash;
    window.location.hash = "#/write?campaign=aiona";
    try {
      await render({
      ...
      // 브랜드 없이 저장해 둔 글이다. 캠페인이 그것을 되돌리면 사용자가 뺀 브랜드가
      // 다시 붙는다.
      expect(container.querySelector<HTMLSelectElement>("#brandId")?.value).toBe("");
    } finally {
      window.location.hash = before;
    }
  });
